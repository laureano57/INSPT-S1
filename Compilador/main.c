...
void cargarByte(memStruct **mem, byte dato) {
...
void cargarInt(memStruct **mem, int dato) {
  int i = (*mem)->topeMemoria;
...
void cargarByteEn(memStruct **mem, byte dato, int pos) {
  (*mem)->bytesArray[pos] = dato;
}

// Carga un int (4 bytes) en el array de memoria en la posicion dada
void cargarIntEn(memStruct **mem, int dato, int pos) {
  unsigned int k = 4294967295; // -1 en 4 bytes (FF FF FF FF)
  unsigned int d;
...
int leerIntDe(memStruct **mem, int pos) {
...
void cargarHeader(memStruct **mem) {
...
tSimbolo programa(tSimbolo s, FILE *archivo, memStruct **memoria) {
  ...
  cargarByte(memoria, 0xBF);
  cargarInt(memoria, 0);  // Inserta 4 bytes en 0

  s = bloque(s, archivo, memoria, tabla, &desplVarMem, 0);
  if (s.simbolo != PUNTO) error(1, s); // Se esperaba un PUNTO
  s = aLex(archivo);
  // Lo que hizo Gomez Insua
  // distanciaFin = 0x588 - (*topememoria + 5); // Calcula la distancia hasta el fin del programa (0x588 finaliza el programa)
  // CargarByte(memoria, topememoria, 0xE9);  // Escribe un salto de memoria (0xE9 es un salto de memoria)
  // CargarInt(memoria, topememoria, distanciaFin);
  // d = leerIntDeMem(memoria, 212) + leerIntDeMem(memoria, 204); // 212 es el size del header, 204 es el size de algo
  // CargarIntEnMem(memoria, 1793, (d + *topememoria - 0x200)); // se carga en la posicion 1793 el size de de la parte text
  // for (int i = 0; i < posicionVar; i++) {
  //   CargarInt(memoria, topememoria, 0); // un 0 de 4 bytes por cada variable
  // }
  // CargarIntEnMem(memoria, 416, *topememoria - 0x200);
  // fileAlignment = leerIntDeMem(memoria, 220);
  // while (*topememoria % fileAlignment != 0) { // Se llena el archivo para que sea multiplo de 512 (requisito de windows)
  ...
  // sectionAlignent = leerIntDeMem(memoria, 216);
  // CargarIntEnMem(memoria, 240, (2 + sizeOfCodeSection / sectionAlignment) * sectionAlignment);
  // CargarIntEnMem(memoria, 208, (2 + sizeOfRawData / sectionAlignment) * sectionAlignment);
  return s;
}
...
tSimbolo bloque(tSimbolo s, FILE *archivo, memStruct **memoria, tablaDeIdent tabla, int *desplVarMem, int base) {
  ...
  cargarByte(memoria, 0xE9);
  cargarInt(memoria, 0);  // JMP en cero, al final se vuelve para corregir el salto
  ...
  if (s.simbolo == VAR) {
    do {
      ...
      strcpy(tabla[base + desplazamiento].nombre, s.cadena);
      tabla[base + desplazamiento].valor = *desplVarMem; // La dirección de memoria a que se refiere la variable (sólo el desplazamiento, es una posicion relativa a EDI)
      *desplVarMem += 4;
      desplazamiento++;
  ...
  switch (s.simbolo) {
    ...
    case READLN:
      ...
      do {
        ...
        s = aLex(archivo);
        // Lo que hizo Gomez Insua
        // distanciaReadln = 0x590 - (*topememoria + 5);
        // CargarByte(memoria, topememoria, 0xE0);
        // CargarInt(memoria, topememoria, distanciaReadln);
