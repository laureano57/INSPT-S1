...
tSimbolo programa(tSimbolo s, FILE *archivo, memStruct *memoria) {
  ...
  int distanciaFin = 0x588 - (memoria->topeMemoria + 5);            // Pag. 31, parrafo 1
  cargarByte(memoria, 0xE9);                                        // Calcula la distancia hasta la instrucción de finalización de programa (0x588)
  ...
  int textSize = imageBase + baseOfCode - 0x700;
                                                                            // Pag. 31, parrafo 3
  int distBase = imageBase + baseOfCode + memoria->topeMemoria - 0x700;     // se carga en la posicion 1793 el size de de la parte text,
  cargarIntEn(memoria, distBase, 1793);                                     // 0x700 es la posicion donde termina el header (en 0x701 / 1793 empieza text)

  for (int i = 0; i < desplVarMem / 4; i++) {                       // Pag. 31, parrafo 4
    cargarInt(memoria, 0);                                          // Carga 1 cero por cada variable (4 bytes, ints de 32 bits)
  }

  cargarIntEn(memoria, memoria->topeMemoria - 0x200, 416);          // Pag. 31, parrafo 5 - Escribe el tamaño de la sección .text en la pos. 416 (virtual size)
  ...
  cargarIntEn(memoria, memoria->topeMemoria - 0x200, 188);          // Pag. 31, parrafo 7
  cargarIntEn(memoria, memoria->topeMemoria - 0x200, 424);          // Ajusta campos SizeOfCodeSection y SizeOfRawData con el tamaño final de .text
    ...
    case WRITE:
      ...
      do {
        ...
        if (s.simbolo == CADENA) {
          ...
          int baseOfCode = leerIntDe(memoria, 204);
          int imageBase = leerIntDe(memoria, 212);
          posCadena = imageBase + baseOfCode + memoria->topeMemoria - 0x200 + 15;
          ...
          cargarInt(memoria, 0x420 - (memoria->topeMemoria + 4));
        }
      } while (s.simbolo == COMA);
